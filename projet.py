...
avg_prices = joined.groupBy("Country").agg(avg(col("Dollar Prices"))).sort("Country").select(
...
moyennePrix = avg_prices.withColumn("Ecart entre la moyenne et celle des USA(%)", round(expr(
    f"(abs(Moyenne) - {usa_average})/ {usa_average} * 100"), 2)).sort(desc("Ecart entre la moyenne et celle des USA(%)"))
